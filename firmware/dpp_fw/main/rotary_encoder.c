...
volatile uint32_t re_last_event_ts[ROTARY_ENCODER_COUNT];
volatile uint8_t re_last_event_dir[ROTARY_ENCODER_COUNT];
...
static void _isr_rotenc(void * args)
{
    ...
    if (send_event && info->queue)
    {
        ...
        // dddddddddddddd
        re_last_event_ts[info->id] = pdTICKS_TO_MS(xTaskGetTickCount());
        re_last_event_dir[info->id] = info->state.direction;
        if (task_woken)
        {
            portYIELD_FROM_ISR();
        }
...
esp_err_t rotary_encoder_init(rotary_encoder_info_t * info, gpio_num_t pin_a, gpio_num_t pin_b, uint8_t device_id)
{
    ...
    if (info)
    {
        ...
        .pin_bit_mask = BIT64(info->pin_a) | BIT64(info->pin_b),
        ...
        ESP_ERROR_CHECK(gpio_config(&rc_cofig));
